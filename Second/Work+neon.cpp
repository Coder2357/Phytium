...
void Sobel_edge_change(Mat input_frame, Mat& output_frame); // Sobel边缘检测函数
...
int main() {
    ...
    while (true) {
        ...
        Sobel_edge_change(sum_frame, sobel_frame);
...
void Sobel_edge_change(Mat input_frame, Mat& output_frame) {
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            for (int c = 0; c < input_frame.channels(); c++) {
                uint8x8_t x_data = vld1_u8(abs_x_ptr + x * input_frame.channels() + c);
                uint8x8_t y_data = vld1_u8(abs_y_ptr + x * input_frame.channels() + c);

                // 将两个寄存器中的数据相加并除以2，然后取整和饱和处理
                uint16x8_t sum = vaddl_u8(x_data, y_data);
                uint8x8_t result_u8 = vqrshrn_n_u16(sum, 1);

                // 将结果写回output_frame的对应位置
                vst1_u8(output_ptr + x * input_frame.channels() + c, result_u8);
            }
        }
    }
}
