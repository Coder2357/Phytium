...
void Hough_lines(Mat input_frame, Mat& output_frame) {
    ...
    //由于仅做Sobel并不理想，这里增加高斯模糊处理、Canny、区域兴趣来得到更好的结果
    // 高斯模糊处理
    ...
    vector<Vec4i> lines;
    HoughLinesP(edges, lines, 1, CV_PI / 180, 50, 30, 5); // Hough直线检测

    cvtColor(edges, output_frame, COLOR_GRAY2BGR); // 转换回彩色图像

    for (size_t i = 0; i < lines.size(); i++) {
        Vec4i l = lines[i];
        //由于车道是有规律的，因此我们可以通过这个过滤一部分线段
        // 计算线段的斜率
...
void final(Mat input_frame, Mat hough_frame, Mat& output_frame) {
    ...
    // 获取图像的行数和列数
    int rows = hough_frame.rows;
    int cols = hough_frame.cols;
